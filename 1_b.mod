...
var InventorioPlanetas{Planetas,Tipo,B} >=0;
var Entregue{Planetas, Tipo, T}>=0;


maximize Lucro:
  sum{p in Planetas, t in Tipo, m in T}
     (Entregue[p, t, m] * Preco[p, t, m])
  - PrecoInventarioTerra * sum {t in Tipo, m in T} InventorioT[t, m]
  - CustoViagem * sum{p in Planetas, m in T} Ativado[p,m]
  - PrecoInventarioPlanetas * sum {p in Planetas,t in Tipo, m in T} InventorioPlanetas[p,t, m];
...
subject to
  ...
  BalancoInventorioP{p in Planetas,t in Tipo, m in T}:
